import { Box, Button, Checkbox, Container, FormControlLabel, FormGroup, IconButton, Input, InputBase, Modal, useMediaQuery, useTheme } from '@mui/material';
...
import { addnewGroup } from '../../../../services/groupService';
...
const ModuleGroup = (props) => {
    ...
    const handleCustomData = () => {
        if (dataredux && dataredux.listfriend && dataredux.listfriend.length > 0) {
            let data = dataredux.listfriend.map((item, index) => {
                return {
                    ...item, checked: false
                }
            })
        ...
        if (arrUser && arrUser.length > 0) {
            ...
            if (choosegroup && choosegroup.length > 0) {
                ...
                if (index !== -1) {
                    ...
                    setusersearch((prevUser) => ({ ...prevUser, checked: false }));
                } else {
                    setusersearch((prevUser) => ({ ...prevUser, checked: false }));
                }
                setarrUser(cparruser)
